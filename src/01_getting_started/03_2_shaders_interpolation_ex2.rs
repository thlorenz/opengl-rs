...
pub fn main() {
    ...
    let shader = Shader::new(
        "src/01_getting_started/03_2_shaders_interpolation_ex2.vert",
    ...
    while !window.should_close() {
        ...
        unsafe {
            ...
            let dx = time * 0.05;
            let dx_cstr = CString::new("dx").unwrap();
            let x_location = gl::GetUniformLocation(shader.id, dx_cstr.as_ptr());
            gl::Uniform1f(x_location, dx);
