...
fn main() {
    ...
    scene.camera = Camera {
        position: glm::vec3(-1.0, 0.9, -1.25),
        pitch: -10.4,
        yaw: -306.0,
        ..Camera::default()
    };
    ...
    let cube_shader = Shader::new(
        "src/ch02_lighting/02_2_basic_specular/cube.vert",
        "src/ch02_lighting/02_2_basic_specular/cube.frag",
    )
    .expect("Failed to create lighting shader");
    let lamp_shader = Shader::new(
        "src/ch02_lighting/02_2_basic_specular/lamp.vert",
        "src/ch02_lighting/02_2_basic_specular/lamp.frag",
    )
